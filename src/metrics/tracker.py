...
class MetricTracker:
    ...
    def update(self, key, value, n=1):
        ...
        Args:
            key (str): metric name. If not already tracked, a new row is
                added on the fly (used e.g. for EER, which is computed
                once per epoch instead of being pre-registered like
                per-batch metrics).
